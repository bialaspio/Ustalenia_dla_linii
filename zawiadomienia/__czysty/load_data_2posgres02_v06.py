...
def get_dresy():
	...
	drop_query = 'DROP TABLE IF EXISTS ADRESY_OK'
	exec_query_commit(drop_query)
	create_query = 'create table ADRESY_OK ( DZ character varying(1500),	WL character varying(1500),	RODZICE character varying(1500),ADRES_CZ2_POP character varying(1500), ADRES_CZ1_POP character varying(1500), plik_csv character varying(1500))'
	exec_query_commit(create_query)
	
	#Stworzenie tablicy na dane z katalogu ADRESY_BIP
	drop_query = 'DROP TABLE IF EXISTS ADRESY_BIP'
	exec_query_commit(drop_query)
	#create_query = 'create table ADRESY_BIP ( DZ character varying(1500),	WL character varying(1500),	RODZICE character varying(1500),ADRES_CZ2_POP character varying(1500), ADRES_CZ1_POP character varying(1500), plik_csv character varying(1500))'
	create_query = 'create table ADRESY_BIP ( DZ character varying(1500),	WL character varying(1500),	RODZICE character varying(1500), plik_csv character varying(1500))'
	exec_query_commit(create_query)
...
#			os.system('pause')
			...
			for wiersz in data['PRAWIDLOWE_ADRESY']:
				if licznik > 1 and len(wiersz) > 1: 
					DZ = str(wiersz[1].encode('cp1250'))
					WL = str(wiersz[4].encode('cp1250'))
					RODZICE = str(wiersz[5].encode('cp1250'))
					ADRES_CZ2_POP = str(wiersz[12].encode('cp1250'))
					ADRES_CZ1_POP = str(wiersz[13].encode('cp1250'))
					#zaladowania danych do bazy   
					
					insert_query = "insert into ADRESY_OK VALUES ('"+DZ+"','"+WL+"','"+RODZICE+"','"+ADRES_CZ2_POP+"','"+ADRES_CZ1_POP+"','"+table_name+"');"
					print insert_query
			...
			for wiersz in data['BIP_NIEPRAWIDLOWE_ADRESY']:
				if licznik > 1 and len(wiersz) > 1: 
					...
					RODZICE = str(wiersz[5].encode('cp1250'))
					#zaladowania danych do bazy   
					insert_query = "insert into ADRESY_BIP VALUES ('"+DZ+"','"+WL+"','"+RODZICE+"','"+table_name+"');"
					exec_query_commit(insert_query)	
				licznik = licznik+1	
		...
		for file_xls_name in files_xls_names:
			...
			for wiersz in data['Arkusz1']:
				if licznik > 1 and len(wiersz) > 1: 
					...
					DATA = str(wiersz[2].encode('cp1250'))
					GODZINA = str(wiersz[3].encode('cp1250'))
					G = str(wiersz[4].encode('cp1250'))
					DDD = str(wiersz[5].encode('cp1250'))
					KW = str(wiersz[6].encode('cp1250'))
					WL = str(wiersz[9].encode('cp1250'))
					RODZICE = str(wiersz[10].encode('cp1250'))
					ADRES_CZ2 = str(wiersz[12].encode('cp1250'))
					ADRES_CZ1 = str(wiersz[11].encode('cp1250'))
					pwpt = str(wiersz[17].encode('cp1250'))
					#zaladowania danych do bazy   
